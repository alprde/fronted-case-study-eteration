...
class Cart extends Component {
    increaseCartPiece = (product, cart, dispatch, e) => {
    ...
    decreaseCartPiece = (product, cart, dispatch, e) => {
        const productKey = `product${product.id}`;
        const products = cart.products;
                                                ...
                                                <div className="row mb-1" key={product.id}>
                                                    <div className="d-flex justify-content-between">
                                                        ...
                                                        <div className="btn-group">
                                                            <button className="btn btn-light btn-md" onClick={this.decreaseCartPiece.bind(this, product, cart, dispatch)}>-</button>
                                                            <button className="btn btn-primary btn-md">{product.piece}</button>
                                                            <button className="btn btn-light btn-md" onClick={this.increaseCartPiece.bind(this, product, cart, dispatch)}>+</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            )
